...
Index your app by running `npm start`, and start debugging in VS Code by pressing `F5` or by clicking the green debug icon. You can now write code, set breakpoints, make changes to the code, and debug your newly modified code—all from your editor.
...
Index your app by running `npm start`, then press `^D` on macOS or `F9` on Windows and Linux or click the green debug icon to start debugging in WebStorm.
...
Now you can rename `src/App.css` to `src/App.scss` and update `src/todoActions.js` to import `src/App.scss`.
...
Finally, import the newly created `.scss` file instead of the default Bootstrap `.css` in the beginning of your `src/todoActions.js` file, for example:
...
4. Add `// @flow` to any files you want to type check (for example, to `src/todoActions.js`).
...
To try it, delete all the code in `src/todoActions.js` and replace it with any of the examples on its website. The [Basic Example](https://reacttraining.com/react-router/web/example/basic) is a good place to get started.
...
We recommend to put the test files (or `__tests__` folders) next to the code they are testing so that relative imports appear shorter. For example, if `App.test.js` and `todoActions.js` are in the same folder, the test just needs to `import App from './App'` instead of a long relative path. Colocation also helps find tests more quickly in larger projects.
...
following in their [`src/todoActions.js`](src/index.js) file:
...
1. [Index a new netlify project](https://app.netlify.com/signup)
...
- If the watcher doesn’t see a file called `todoActions.js` and you’re referencing it by the folder name, you [need to restart the watcher](https://github.com/facebook/create-react-app/issues/1164) due to a Webpack bug.
